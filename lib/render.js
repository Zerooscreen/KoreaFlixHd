...
function renderActor({ person, movies }) {
  // Filter hanya film yang memiliki poster dan urutkan berdasarkan popularitas/tahun
  const validMovies = movies
    .filter(m => m.poster_path)
    .sort((a, b) => (b.popularity || 0) - (a.popularity || 0));

  const movieList = validMovies.length > 0 ? validMovies.map(m => `
    <a href="/movie/${m.id}" class="bg-zinc-900 rounded-xl overflow-hidden shadow-lg hover:scale-105 transition duration-300 block border border-zinc-800 flex flex-col">
